...
<?php  if ($_REQUEST['approveImmediately']) { ?>
...
<input type="hidden" name="approveImmediately" value="<?php echo $_REQUEST['approveImmediately']?>" />
<input type="hidden" name="rel" value="<?php echo $_REQUEST['rel']?>" />
	...
	<ul class="tabs" id="ccm-properties-tabs">
		...
		<li <?php  if ($c->isMasterCollection()) { ?>style="display: none"<?php  } ?>><a href="javascript:void(0)" id="ccm-page-paths"><?php echo t('Page Paths and Location')?></a></li>
	</ul>
	<?php  } ?>
	...
	<div class="input"><textarea id="cDescription" name="cDescription" class="ccm-input-text" style="width: 495px; height: 50px"><?php echo $c->getCollectionDescription()?></textarea></div>
	</div>
