...
<?php  if ($displayEditLink) { ?>
	<label><a href="javascript:void(0)" onclick="ccm_composerEditBlock(<?php echo $b->getBlockCollectionID()?>, <?php echo $b->getBlockID()?>, '<?php echo $b->getAreaHandle()?>', <?php echo $bt->getBlockTypeInterfaceWidth()?> , <?php echo $bt->getBlockTypeInterfaceHeight()?> )" ><?php echo $btName?></a></label>
<?php  } else { ?>
	<label><?php echo $btName?></label>
<?php  } ?>
