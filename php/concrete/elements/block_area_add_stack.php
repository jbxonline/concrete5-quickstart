	...
	<?php  if (count($stacks) > 0) { 
		foreach($stacks as $s) { 
			...
			<li class="ccm-stack-available">
				<a onclick="ccmStackAddToArea(<?php echo $s->getCollectionID()?>, '<?php echo Loader::helper('text')->entities($a->getAreaHandle())?>')" href="javascript:void(0)"><?php echo $s->getCollectionName()?></a>
			</li>
