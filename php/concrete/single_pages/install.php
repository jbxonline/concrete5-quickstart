...
<script type="text/javascript" src="<?php echo ASSETS_URL_JAVASCRIPT?>/bootstrap.twipsy.js"></script>
...
<?php  for ($i = 1; $i <= count($installRoutines); $i++) {
	...
	ccm_installRoutine<?php echo $i?> = function() {
		<?php  if ($routine->getText() != '') { ?>
			$("#install-progress-summary").html('<?php echo $routine->getText()?>');
		<?php  } ?>
		$.ajax('<?php echo $this->url("/install", "run_routine", $installPackage, $routine->getMethod())?>', {
			...
			success: function(r) {
				if (r.error) {
					...
					$( "#install-progress-bar" ).progressbar({
						value: <?php echo $routine->getProgress()?>
					});
...
<div class="span8 columns">
	...
	<fieldset>
		...
		<div class="clearfix">
		<label for="SITE"><?php echo t('Name Your Site')?>:</label>
		<div class="input">
...
<h3><?php echo t('Sample Content')?></h3>
		...
		<tr class="<?php  if ($this->post('SAMPLE_CONTENT') == $pkgHandle || (!$this->post('SAMPLE_CONTENT') && $pkgHandle == 'standard') || count($availableSampleContent) == 1) { ?>package-selected<?php  } ?>">
			...
			<td><img src="<?php echo $uh->getPackageIconURL($spl)?>" width="97" height="97" alt="<?php echo $spl->getPackageName()?>" /></td>
			<td width="100%"><h4><?php echo $spl->getPackageName()?></h4><p><?php echo $spl->getPackageDescription()?></td>
		</tr>
...
<div class="well">
	<?php echo $form->submit('submit', t('Install concrete5'), array('class' => 'large primary'))?>
</div>
</form>
...
<tr>
	...
	<td><?php  if (!$phpVtest) { ?><img src="<?php echo ASSETS_URL_IMAGES?>/icons/tooltip.png" class="tooltip" title="<?php echo t('While concrete5 will mostly run on PHP 5.1, 5.2 is strongly encouraged and some functions will not work properly without it.')?>" /><?php  } ?></td>
</tr>
<tr>
...
<tr>
	...
	<td><?php  if (!$mysqlTest) { ?><img src="<?php echo ASSETS_URL_IMAGES?>/icons/tooltip.png" class="tooltip" title="<?php echo $this->controller->getDBErrorMsg()?>" /><?php  } ?></td>
</tr>
<tr>
	...
	<td><img id="ccm-test-request-tooltip" src="<?php echo ASSETS_URL_IMAGES?>/icons/tooltip.png" class="tooltip" title="<?php echo t('concrete5 cannot parse the PATH_INFO or ORIG_PATH_INFO information provided by your server.')?>" /></td>
</tr>
</table>
...
<tr>
	...
	<td><?php  if (!$imageTest) { ?><img src="<?php echo ASSETS_URL_IMAGES?>/icons/tooltip.png" class="tooltip" title="<?php echo t('concrete5 requires GD library 2.0.1 or greater')?>" /><?php  } ?></td>
</tr>
<tr>
	...
	<td><?php  if (!$xmlTest) { ?><img src="<?php echo ASSETS_URL_IMAGES?>/icons/tooltip.png" class="tooltip" title="<?php echo t('concrete5 requires PHP XML Parser and SimpleXML extensions')?>" /><?php  } ?></td>
</tr>
<tr>
	...
	<td width="100%"><?php echo t('Web Server Access to Files and Configuration Directories')?>
	</td>
	<td><?php  if (!$fileWriteTest) { ?><img src="<?php echo ASSETS_URL_IMAGES?>/icons/tooltip.png" class="tooltip" title="<?php echo t('The config/, packages/ and files/ directories must be writable by your web server.')?>" /><?php  } ?></td>
</tr>
...
<tr>
	...
	<td><?php  if (!$remoteFileUploadTest) { ?><img src="<?php echo ASSETS_URL_IMAGES?>/icons/tooltip.png" class="tooltip" title="<?php echo t('Remote file importing through the file manager requires the iconv PHP extension.')?>" /><?php  } ?></td>
</tr>
</table>
...
<tr>
	<td><?php  if ($diffTest) { ?><img src="<?php echo ASSETS_URL_IMAGES?>/icons/success.png" /><?php  } else { ?><img src="<?php echo ASSETS_URL_IMAGES?>/icons/warning.png" /><?php  } ?></td>
	<td width="100%"><?php echo t('Version Comparison Available')?>
	</td>
	<td><?php  if (!$diffTest) { ?><img src="<?php echo ASSETS_URL_IMAGES?>/icons/tooltip.png" class="tooltip" title="<?php echo t('You must chmod 755 %s and disable PHP safe mode.', 'concrete/libraries/3rdparty/htmldiff.py')?>" /><?php  } ?></td>
</tr>
</tbody>
</table>
</div>
...
<div class="well" id="install-success">
	...
	<input type="hidden" name="locale" value="<?php echo $locale?>" />
	<?php echo $form->submit('install', t('Continue to Installation'), array('class' => 'large primary'))?>
	</form>
</div>
...
<div class="block-message alert-message error" id="install-errors">
	...
	<input type="hidden" name="locale" value="<?php echo $locale?>" />
	<?php echo $form->submit('rerun', t('Run Tests'), array('class' => 'small'))?>
	</form>
	</div>	
</div>
