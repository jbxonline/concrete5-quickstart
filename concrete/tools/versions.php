<?php 
	...
	if ($_GET['vtask'] == 'view_version') { ?>
		...
		*/
		...
		<iframe border="0" id="v<?php echo time()?>" frameborder="0" height="100%" width="100%" src="<?php echo BASE_URL . DIR_REL?>/<?php echo DISPATCHER_FILENAME?>?cvID=<?php echo $_REQUEST['cvID']?>&cID=<?php echo $_REQUEST['cID']?>&vtask=view_versions" />
...
<div class="ccm-form-area">
	...
	if (!$ih->inDashboard($c)) { ?>
		&nbsp;&nbsp;
		<input class="btn" type="button" name="vCompare" value="<?php echo t('Compare')?>" disabled />
	&nbsp;
	<?php  } ?>
	<input class="btn" type="button" name="vApprove" value="<?php echo t('Approve')?>" disabled />
	
	&nbsp;
	<input class="btn" type="button" name="vRemove" value="<?php echo t('Remove')?>" disabled />
	...
	<tr>
		...
		<th class="headerSortDown"><?php echo t('Added On')?></th>
	</tr>
	<?php  
	$vIsPending = true;
	foreach ($vArray as $v) { 
	...
	<tr id="ccm-version-row<?php echo $v->getVersionID()?>" class="<?php echo $class?>">
		...
		<td><a dialog-width="85%" dialog-height="80%" title="<?php echo t('Compare Versions')?>" class="ccm-version" dialog-title="<?php echo t('Compare Versions')?>" dialog-modal="false" href="<?php echo REL_DIR_FILES_TOOLS_REQUIRED?>/versions.php?cID=<?php echo $cID?>&cvID=<?php echo $v->getVersionID()?>&vtask=view_version"><?php echo $v->getVersionName()?></a></td>
