...
<?php  if (!$f->overrideFileSetPermissions()) { ?>
	<p><?php echo t('Permissions for this file are currently dependent on set and global settings. If you override those permissions here, they will not match those of the file\'s sets.')?></p>
<?php  } else { ?>
	<p><?php echo t("Permissions for this file currently override file set and global settings. To revert these permissions, click the button below.")?></p>
<?php  } ?>	
</div>
</div>
<div class="clearfix">
...
?>
	...
	<?php  if ($f->overrideFileSetPermissions()) { ?>
		...
		<a href="javascript:void(0)" onclick="$('#fRevertToSetPermissions').val(1);ccm_alSubmitPermissionsForm('<?php echo $searchInstance?>')" class="ccm-button-left cancel btn"><?php echo t('Revert Permissions')?></a>
	<?php  } ?>
	<?php echo $ih->button_js(t('Update'), 'ccm_alSubmitPermissionsForm(\'' . $searchInstance . '\')', 'right', 'primary')?>
	</div>
