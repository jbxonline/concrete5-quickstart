...
<div id="confirmDelete" style="display:none" class="ccm-ui">
		...
		<div class="dialog-buttons">
			...
			<?php echo  $interface->submit('Delete Backup', false, 'right', 'error'); ?>
	</form>
		</div> 
</div>
...
<div id="confirmRestore" style="display:none" class="ccm-ui">
		...
		<div class="dialog-buttons">
			...
			<input type="hidden" name="backup_file" value="" />
			<?php echo  $interface->submit('Restore Backup', false, 'right', 'primary'); ?>
		</form>
		</div> 
</div>
...
if ($tp->canBackup()) {
	?>
	<?php echo  Loader::helper('concrete/dashboard')->getDashboardPaneHeaderWrapper(t('Backup Database'), false, 'span14 offset1') ?>
			...
			<table class="zebra-striped" cellspacing="1" cellpadding="0" border="0">
				...
				<tbody>
					<?php  foreach ($backups as $arr_bkupInf) { ?>
						<tr> 
							<td width="50%" style="white-space: nowrap"><?php echo  date(DATE_APP_GENERIC_MDYT_FULL, strtotime($arr_bkupInf['date'])) ?></td>
			...
			<h3><?php echo t('Create new Backup')?></h3>
				<form method="post" action="<?php echo  $this->action('run_backup') ?>">
					<div class="ccm-buttons well">
						<div style="float: left;"><?php echo  $interface->submit(t("Run Backup"), false, "left") ?></div>
...
<?php  } else { ?>
	<?php echo  Loader::helper('concrete/dashboard')->getDashboardPaneHeaderWrapper(t('Backup'), false, 'span6 offset6') ?>
