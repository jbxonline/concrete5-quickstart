...
if ($downloadableUpgradeAvailable) { ?>
	<?php echo $h->getDashboardPaneHeaderWrapper(t('Download Update'));?>
...
<?php  } else if (count($updates)) { ?>
	<?php echo $h->getDashboardPaneHeaderWrapper(t('Install Local Update'),false,false,false);?>
...
<?php  } else { ?>
	<?php echo $h->getDashboardPaneHeaderWrapper(t('Update concrete5'));?>
	<?php  if (!defined('MULTI_SITE') || MULTI_SITE == false) { ?>
		<a href="<?php echo $this->action('check_for_updates')?>" class="btn" style="float: right"><?php echo t('Check For Updates')?></a>
	<?php  } ?>
		<h2><?php echo t('Currently Running %s',config::get('SITE_APP_VERSION'))?></h2>
